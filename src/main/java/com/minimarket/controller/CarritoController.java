...
public class CarritoController {
    ...
    @PreAuthorize("hasRole('CLIENTE')") // Protegido: Solo CLIENTE puede agregar
    @PostMapping
    public Carrito agregarProductoAlCarrito(@RequestBody Carrito carrito) {
        return carritoService.save(carrito);
    }

    @PreAuthorize("hasRole('CLIENTE')") // Protegido: Solo CLIENTE puede actualizar
    @PutMapping("/{id}")
    ...
    @PreAuthorize("hasRole('CLIENTE')") // Protegido: Solo CLIENTE puede eliminar
    @DeleteMapping("/{id}")
